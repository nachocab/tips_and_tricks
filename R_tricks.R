...
legend("bottomright", levels(timepoints), lty=1, lwd=5, col=seq(along=levels(timepoints)))
...
unordered_factor <- relevel(unordered_factor, "Ref") # relevel with a new reference value
...
target <- letters
x <- match(query,target) #  7  1  4 NA NA  6
x <- x[!is.na(x)] #  7  1  4 6
match("a", c("a","b","c")) # 1
...
text(df_text$x, df_text$y, labels=df_text$gene_symbol, pos=4) # pos 1: right-aligned 2: bottom 3: top 4: left-aligned
...
[1] 42
x <- 42
deparse(substitute(x)) # get name of a variable, variable name
# "x"
...
# suppress output
sink("/dev/null"); expr(); sink(); # sink() un-sinks
sink("my_file"); expr(); sink();
invisible(expr())
...
do.call("paste", c(tmp, sep="")) # pass arguments to FUN, tmp is already a list or a data.frame
...
legend("topright", levels(categories), lty=1, lwd=5, col=seq(along=levels(categories)))
legend(10,40, rownames(rates), lty=1, lwd=5, col=c("steelblue", "red"))
legend(10,40, rownames(rates), pch=16, col=c("steelblue", "red"))
...
pcp_cl + geom_line(alpha = .75) + theme(axis.title.x=element_blank()) + facet_wrap(~ cluster)
...
trialm <- melt(trial)
trialm <- cbind(trialm, colsplit(trialm$variable, split="", names = c("treatment", "time")))
...
# ROCR, but use pROC because it's cooler
library(ROCR)
